...
The part I enjoyed was watching the bottleneck move. Once the multiplications were cheap, the final division was the slow step. Go's division is Karatsuba-bound too, so I wrote a Newton reciprocal that does the division with FFT multiplies instead. Then the square root became the slowest step, since the formula needs √10005, so that got the same treatment with a Newton inverse square root. Every fix just uncovered the next thing in line, until the whole pipeline was running on FFT.
