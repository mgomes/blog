...
The default way you run a Vibescript, the `xhigh` profile, does not set a memory quota at all, so the accounting is switched off entirely. That 10.5 seconds of `fib(35)` on my M1 Max is pure tree-walking, not a cent of sandbox tax. Turn a memory quota on and the same `fib(35)` takes about 50 seconds on this machine (~5x slower) and 28 seconds on the M4 (~4x slower), while its actual memory footprint never budges from about 5 MB. The 4-5x is the accounting, and the whole push above is what turned it from "minutes, or would not run at all" into "five times slower," and on the collection-heavy scripts people actually write, into almost nothing.
